...
while True:
    if cont < 2:
        custokwh = float(input('\n' f'Digite o custo do kWh em sua região [DIGITE 999 PARA CALCULAR PELA CONTA '
                               f'DE ENERGIA] {verde}R${acaba}'))
        if custokwh == 999:
    ...
    dias = ' '
    while dias[0] not in 'SN' or not dias.isalpha():
    ...
    print('=' * 94)
    continuar = ' '
    while continuar[0] not in 'SN' or not continuar.isalpha():
...
for c in range(0, cont - 1):
    print(f'\nO aparelho {vermelho}{aparelhocont[c]}{acaba} com uma potência de {potenciacont[c]} watts ligado '
          f'{tempocont[c]:.0f} horas por {diascontador[c]:.0f} dias vai sair por {verde}R${precocont[c]:.2f}{acaba}\n')
print('=' * 94)
